...
import { removeTrackToPlayer, setPlayerHeight } from '../../store/actions';
...
const Player = () => {
  ...
  return (
    <div ref={playerRef} className={`player ${currentTrack ? 'is-playing' : ''}`} data-testid="player" >
      {currentTrack && (
        <div className="player__wrapper">
          <div className="player__progress-bar">
            <div className="player__progress-bar__stroke" style={{width: `${progressBarWidth}`}} />
          </div>

          <div className="container">
            <figure className="player__album-cover" style={{ backgroundImage: `url(${currentTrack.album?.images[1]?.url || ''})` }}/>
          ...
          <audio ref={audioElementRef} autoPlay onEnded={handleOnEnded} onTimeUpdate={handleTimeUpdate} preload="metadata" src={currentTrack.preview_url} />
        </div>
      )}
    </div>
  );
